...
app.post("/changePoints", async (req, res) => {
  ...
  var tpoints=0,q1=0,q2=0,q3=0,q4=0,curr_yr_points = 0, prev_yr_points =0;
  ...
  for (const i of await Mails.find({ email })) {
    ...
    //quater calculation
    if(year === curr_year){
    if(month_name==="April"||month_name==="May"||month_name==="June"){
      q1 =q1+cp.community_points;
   }else if(month_name==="July"||month_name==="August"||month_name==="September"){
    q2=q2+cp.community_points;
   }else if(month_name==="October"||month_name==="November"||month_name==="December"){
    q3=q3+cp.community_points;
   }else if(month_name==="January"||month_name==="February"||month_name==="March"){
